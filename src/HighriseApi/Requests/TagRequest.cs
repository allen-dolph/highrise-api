...
namespace HighriseApi.Requests
{
    public class TagRequest : RequestBase, ITagRequest
    {
        ...
        {
            var response = Client.Execute<List<Tag>>(new RestRequest("tags.xml", Method.GET));
            return response.Data;
        }

        public IEnumerable<Tag> Get(SubjectType subjectType, int subjectId)
        {
            var response = Client.Execute<List<Tag>>(new RestRequest(String.Format("{0}/{1}/tags.xml", subjectType.ToString().ToLower(), subjectId), Method.GET));
            return response.Data;
        }

        public Parties Parties(int tagId)
        {
            var response = Client.Execute(new RestRequest(String.Format("tags/{0}.xml", tagId), Method.GET));
        ...
        public Tag Create(SubjectType subjectType, int subjectId, Tag tag)
        {
            ...
            var response = Client.Execute<Tag>(request);
            return response.Data;
        }

        public bool Delete(SubjectType subjectType, int subjectId, int tagId)
        {
            var response = Client.Execute<Tag>(new RestRequest(String.Format("{0}/{1}/tags/{2}.xml", 
