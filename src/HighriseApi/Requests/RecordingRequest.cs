...
namespace HighriseApi.Requests
{
    public class RecordingRequest : RequestBase,IRecordingRequest
    {
        ...
        {
            var response = Client.Execute<List<Recording>>(new RestRequest("recordings.xml", Method.GET));
        ...
        public IEnumerable<Recording> Get(DateTime startDate)
        {
            ...
            var response = Client.Execute<List<Recording>>(new RestRequest(url, Method.GET));
            return response.Data;
        }
    }
}
